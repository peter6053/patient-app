...
class PatientRegistrationViewModel @Inject constructor(
    ...
    fun validateAndRegister() {
        when {
            _firstName.value.isBlank() -> _state.value = Resource.Error("First name is required")
            _lastName.value.isBlank() -> _state.value = Resource.Error("Last name is required")
            _dob.value.isBlank() -> _state.value = Resource.Error("Date of birth is required")
            _gender.value.isBlank() -> _state.value = Resource.Error("Gender is required")
            _regDate.value.isBlank() -> _state.value = Resource.Error("Registration date is required")
    ...
    fun registerPatient() {
        viewModelScope.launch {
            ...
            try {
                ...
                val localEntity = RegisterPatientEntity(
                    id = _patientId.value,
                    dob = _dob.value,
                    firstname = _firstName.value,
                    gender = _gender.value,
                ...
                patientDao.insertPatient(localEntity)
                ...
                // Handle offline scenario or DB insert error
                _state.value = Resource.Error("Saved offline: ${e.message ?: "Unknown error"}")
            }
        }
    }
}
